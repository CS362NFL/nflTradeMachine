...
public class NFLTradeMachine {
	...
	public static void main(String[] args) {
		...
		while (!input.equals("Exit") && !input.equals("exit")) {
			System.out
					.println("Please enter one of the following commands: trade, createPlayer, getTeam, getPlayer, getPlayersFromTeam, addPlayerToTeam, reset, exit");
	...
	public static void trade() {
		...
		while (p1 == null) {
			...
			List<Player> players = nflManagerController
					.searchPlayersByName(firstPlayerName);
			if (players.size() == 0) {
		...
		boolean isValidTrade = nflManagerController.evaluate(p1.getId(),
				p2.getId());

		if (!isValidTrade) {
			System.out.println("Trade is not valid!");
			if (p1.getTeam().getFreeCapSpace() + p1.getAverageCapHit() < p2
					.getAverageCapHit())
				System.out.println(p1.getTeam().getName()
						+ " do not have enough cap space for " + p2.getName());
			if (p2.getTeam().getFreeCapSpace() + p2.getAverageCapHit() < p1
					.getAverageCapHit())
				System.out.println(p2.getTeam().getName()
						+ " do not have enough cap space for " + p1.getName());
		}

		else {
			Team winner = nflManagerController.evaluateWinner(p1.getId(),
					p2.getId());
	...
	public static void getTeam() {
		...
		for (Team t : nflManagerController.getTeams()) {
			if (t.getName().toLowerCase().contains(teamName.toLowerCase())) {
				System.out.println(t.getName() + "\t" + t.getSalaryCap() + "\t"
						+ t.getFreeCapSpace());
			}
		}
	}
	...
	public static void getPlayer() {
		System.out
				.println("Would you like to search by Name, Position, or Average Cap Hit");
		String option = scanner.nextLine();
		List<Player> players = new ArrayList<Player>();
		if (option.toLowerCase().contains("Name".toLowerCase())) {
			System.out.println("Enter a Name to search for:");
			players = nflManagerController.searchPlayersByName(scanner
					.nextLine());
		} else if (option.toLowerCase().contains("Position".toLowerCase())) {
			System.out.println("Enter a Position to search for:");
			players = nflManagerController.searchPlayersByPosition(scanner
					.nextLine());
		} else if (option.toLowerCase().contains(
				"Average Cap Hit".toLowerCase())) {
			System.out.println("Enter the low and high bounds to search for:");
			String[] values = scanner.nextLine().split(" ");
			players = nflManagerController.searchPlayersByAverageCapHit(
	...
	public static void createPlayer() {
		System.out
				.println("Please enter the name of the player you wish to create: ");
		String playerName = scanner.nextLine();
		System.out
				.println("Please enter the position of the player you wish to create: ");
	...
	public static void addPlayerToTeam() {
		...
		while (p == null) {
			System.out
					.println("Please enter player you wish to add to a team:");
			String playerName = scanner.nextLine();
			List<Player> players = nflManagerController
					.searchPlayersByName(playerName);
			if (players.size() == 0) {
		...
		if (p.getTeam() != null) {
			System.out.println(p.getName() + " already belongs to the "
					+ p.getTeam().getName());
			return;
		}
		...
		while (t == null) {
			System.out
					.println("Please enter name of team you want to add the player to:");
			String teamName = scanner.nextLine();
			List<Team> teams = nflManagerController.getTeams();
			for (Team team : teams) {
				if (team.getName().toLowerCase()
		...
		p.addTeam(t);
		t.addPlayer(p);
		System.out.println(p.getName() + " successfully added to "
				+ t.getName());
